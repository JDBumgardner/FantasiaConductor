...
job.json (written by fantasia_core.tune.start):
  ...
  stops       distance targets, default ladder.DISTS; n_start (8); device ("mps")
...
def main(job_path):
    ...
    if job.get("stops"): ladder.DISTS = tuple(job["stops"]) + (None,)
    ...
    result = {"text": text, "anchor": anchor, "route": route, "coverage": coverage, "frozen": graph.frozen, "seconds": round(time.time() - t0), "stops": []}
    for i, st in enumerate(stops):
        row = {"stop": i, "target": st.get("target"), "word": round(st["clap"], 4), "distance": round(st["dist"], 3), "identity": round(st["self"], 3), "flags": st.get("flags", [])}
        if i > 0:
