...
DISTS = (0.15, 0.4, 0.8, 1.5, 3.0, None)                      # None = unconstrained: as far as the priors and the artefact penalties allow
MU, ART_W = 5.0, float(os.environ.get("T2_ART_W", "1.0"))
ROUNDS_FIRST, ROUNDS_NEXT, ROUNDS_LAST = ((20, 4), (40, 2), (100, 1)), ((80, 1),), ((150, 1),)

def run(word, source, notes, out_dir, tag, anchor_text, synth=None, p_inst=None, n_start=8, log=print):
    """source: (L,) audio of the untouched sound (the recording, or the twin's dry render). synth/p_inst: the twin route."""
    ...
    def loss_of(c):
        ...
        return -(C.embed(w) @ T.T).squeeze() + T2.LAM_PRI * T2.fx_prior(c.pf) + T2.LAM_GS * gs + con + ART_W * A.penalty(out, y, notes)
    def evaluate(c):
        with torch.no_grad():
    ...
    def cand(r):
        if synth is None: return Candidate({}, T2.init_fx(r), label=f"c{r}")
        ...
        return Candidate(ps, T2.init_fx(r), label=f"c{r}")
    ...
    for i, d in enumerate(DISTS):
        ...
        f = os.path.join(out_dir, f"{tag}__{word}__stop{i+1}_d{d if d is not None else 'inf'}.wav"); sf.write(f, (out / (out.abs().max() + 1e-9) * 0.5).cpu().numpy(), SR); wavs.append(f)
        json.dump({"word": word, "tag": tag, "target": d, **{k: v for k, v in ev.items() if k != "score"}, "describe": T2.CHAIN.describe(c.pf), **({"raw": {k: float(v) for k, v in c.ps.items()}} if synth is not None else {}),
    ...
    # the listening ladder: first bar of the original, then every stop
    seg, fade, gap = int(2.4 * SR), int(0.01 * SR), np.zeros(int(0.25 * SR), dtype=np.float32)
    def take(y):
        y = np.asarray(y[:seg], dtype=np.float32); y = y / (np.abs(y).max() + 1e-9) * 0.5; w = np.ones_like(y); w[:fade] = np.linspace(0, 1, fade); w[-fade:] = np.linspace(1, 0, fade); return y * w
    lad = [take(source.cpu().numpy()), gap]
