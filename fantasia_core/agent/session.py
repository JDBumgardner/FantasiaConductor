...
SYSTEM = (
    ...
    "state and the ids you need, then make edits. To write music: add_track, set it to "
    "is_drum or is_synth if wanted, add_clip to make an empty clip, then write_midi with "
    "notes. Note times are seconds from the clip start; use get_project's seconds_per_beat "
    "to place notes on the grid. Drum pitches (on a drum track): 36 kick, 38 snare, 42 "
    "closed hat, 46 open hat, 49 crash. Keep velocities musical (accent beats louder).\n\n"
    "Designing sounds (synth tracks): set a track is_synth, then design its patch with "
    "set_synth_patch(track_id, {params}). How the params sound:\n"
    "- osc1/osc2 waveform: sine=pure, saw=bright/buzzy, square=hollow/reedy, triangle=soft. "
    "mix 0=osc1 only … 1=osc2 only; detune ~0.1-0.3 semitones fattens the tone.\n"
    ...
    "lowpass/highpass for space and character.\n\n"
